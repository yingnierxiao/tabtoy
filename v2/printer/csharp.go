	...
	{{end}}
		...
		public static void Deserialize( {{.Name}} ins, tabtoy.DataReader reader )
		{
			...
			for( int i = 0;i< ins.{{$row.FieldDescriptor.Name}}.Count;i++)
			{
				...
				if(!ins._{{$row.FieldDescriptor.Name}}By{{$key.Name}}.ContainsKey(element.{{$key.Name}})){
					ins._{{$row.FieldDescriptor.Name}}By{{$key.Name}}.Add(element.{{$key.Name}}, element);
				}else{
					UnityEngine.Debug.LogWarningFormat("{{$row.FieldDescriptor.Name}}By{{$key.Name}}, repeat ID: {0}", element.{{$key.Name}});
				}
				{{end}}
			}
			{{end}}
